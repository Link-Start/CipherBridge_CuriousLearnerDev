...
_CRYPTO_KW = re.compile(
    r"encrypt|decrypt|CryptoJS|AES|DES|SM4|password|username|cipher|\.iv|padding|ecb|cbc",
    re.I,
)
...
def _select_scripts_text(scripts: dict[str, str], max_files: int = 8, max_chars: int = 14000) -> str:
    if not scripts:
        return "(无页面 JS)"
    ...
    for url, content in scripts.items():
        score = 0
        low_url = url.lower()
        for kw in ("encrypt", "crypto", "login", "auth", "index", "app", "main", "chunk"):
            if kw in low_url:
                score += 4
        hits = _CRYPTO_KW.findall(content[:10000])
        score += min(len(hits), 30)
        if score > 0:
            ranked.append((score, url, content))
    if not ranked:
        for url, content in list(scripts.items())[:3]:
            ranked.append((1, url, content))
...
def build_analysis_prompt(
    ...
    hooks_text = "\n".join(hook_lines[-120:]) if hook_lines else "(无 Hook 日志 — 请确认已勾选 JS Hook 并在页面执行登录/加密操作)"
    ...
    return [
        {"role": "system", "content": system_prompt_for_role(role)},
        {
            ...
            "content": build_analysis_prompt(
                flows, hook_lines, role, scripts=scripts, focus_hook=focus_hook,
            ),
        },
    ]
